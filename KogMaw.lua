...
KogMawMenu.Mana:Slider("W", "Stop W when % MP", 40, 1, 100, 1)
...
local Mana = (GetCurrentMana(myHero)/GetMaxMana(myHero))*100 --This will get the mana percentage u current have
	...
	if CanUseSpell(myHero,_W) == READY and KogMawMenu.Combo.Combo1:Value() and Mana >= KogMawMenu.Mana.W:Value() and KogMawMenu.Combo.W:Value() and GoS:ValidTarget(target, 600) then
		CastSpell(_W)
	end
